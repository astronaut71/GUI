...
while run_ok == 1
	...
	if isempty(msg)
        ...
        %ctr = 0;
        run_ok = 0;
         if exist('datapredict1.mat') > 0
            data = [ cell2mat(struct2cell(load('datapredict1.mat'))); msg.data' ];
        else
            data = msg.data';
         end
		% save can also be called using "function" format
         save('datapredict1.mat', 'data');    
	end
end

%load('/home/bojan/Desktop/SVM/POWSVM/Data/guitest.mat');
% load ('/home/bojan/Desktop/SVM/POWSVM/Data/forw100runs.mat');ova bese ok
load ('turn180DataLabel.mat');
...
% feature_training= forw100runs(:,1:end-1); class_training =forw100runs(:,end); ova bese ok
